...
#player = {"name": "Player", "level": 1, "attack": 2, "defense": 1, "pv": 10}
...
def end_game():
    #print(player["pv"])
    player["attack"] = player["attack"] + random.randint(1, 3)
    player["defense"] = player["defense"] + random.randint(1, 3)
    back = str(player)
    with open("backup.txt", "w") as fichier:
        fichier.write(back)
    exit(0)
    ...
    while damage != 0:
        if robot["defense"] <= 0:
            if robot["pv"] <= 0:
                for i in range(3):
                    print("")
                print("Partie terminée, défaite de", robot["name"])
                end_game()
                game_end = True
                return 0
            robot["pv"] = robot["pv"] - 1
